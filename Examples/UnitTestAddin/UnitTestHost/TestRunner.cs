...
namespace UnitTestHost
{
    public partial class TestRunner : Form
    {
        ...
        SelectionStorage FStorage = null;
        ...
        private bool RunTests()
        {
            ...
            try
            {
                ...
                //run the unit tests
                for (int LGroupIter = 0; LGroupIter < lstTests.Groups.Count; ++LGroupIter)
                {
                    var LCurrentGroup = lstTests.Groups[LGroupIter];
                    var current = (Type)LCurrentGroup.Tag;
                    try
                    {
                        bool LSomeTestSelected = false;
                        for (int LItemIndex = 0; LItemIndex < LCurrentGroup.Items.Count; ++LItemIndex)
                        {
                        ...
                        string title = String.Format("{0}: {1} test(s)", current.FullName, LCurrentGroup.Items.Count);
                        WriterHeader(title);
                        WriteLogLine();
                        ...
                        {
                            // 4. Run test methods.
                            for (int LMethodIter = 0; LMethodIter < LCurrentGroup.Items.Count; ++LMethodIter)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    {
                                        WriteLogLine(string.Format("  Failed: {0} ({1} s).", ex.InnerException.Message, stats.LocalTime.TotalSeconds));
                                        continue;
                                    }
        ...
        private void lstTests_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string LMethodClass = LSelectedItem.SubItems[1].Text;
            string LMethodName = LSelectedItem.SubItems[2].Text;
            string LFoundText = string.Format("Running test: {0}.{1}",
